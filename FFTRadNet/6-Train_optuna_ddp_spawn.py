...
def objective(single_trial, config, rank,world_size):
    ...
    for epoch in range(num_epochs):
        ...
        ave_train_loss = train_loss_tensor.item() / len(train_data.dataset)  # Optionally average the loss
        val_loss_tensor = torch.tensor([val_loss], dtype=torch.float).to(rank)
        dist.all_reduce(val_loss_tensor)
        ave_val_loss = val_loss_tensor.item() / len(val_data.dataset)  # Optionally average the loss
