...
class RADIalMAT(Dataset):
    ...
    def __init__(self, root_dir,statistics=None,encoder=None,difficult=False):
        ...
        self.labels = pd.read_csv(os.path.join('/imec/other/ruoyumsc/users/chu/data/','filtered_labels.csv')).to_numpy()
    ...
    def __getitem__(self, index):
        ...
        mat_file = os.path.join(self.root_dir, 'RD_cube/', self.mat_files[index])
        print('getting mat file', mat_file)
        idx = index
        mat_input_4d = sio.loadmat(mat_file)['radar_data_cube_4d']
        input = mat_input_4d[:, :, 0, :]
        ...
        return radar_FFT,out_label, idx