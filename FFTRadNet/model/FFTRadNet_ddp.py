...
class MIMO_PreEncoder(nn.Module):
    def __init__(self, in_layer,out_layer,NbVirtualAntenna, kernel_size=(1,12),dilation=(1,16),use_bn = False):
...
class FPN_BackBone(nn.Module):
    ...
    def __init__(self, num_block,channels,block_expansion,mimo_layer,NbTxAntenna,NbRxAntenna,use_bn=True):
        ...
        self.pre_enc = MIMO_PreEncoder(NbRxAntenna*2,mimo_layer,NbTxAntenna*NbRxAntenna,
...
class FFTRadNet(nn.Module):
    ...
    def __init__(self,mimo_layer,Ntx, Nrx, channels,blocks,regression_layer = 2, DH_num_layers=4, detection_head=True):
        ...
        self.FPN = FPN_BackBone(num_block=blocks,channels=channels,block_expansion=4, mimo_layer = mimo_layer, NbTxAntenna=Ntx, NbRxAntenna=Nrx, use_bn = True)
