...
class Detection_Header(nn.Module):
    ...
    def __init__(self, use_bn=True,reg_layer=2,input_angle_size=0):
        ...
        if(self.input_angle_size==224):
            ...
            self.bn1 = nn.BatchNorm2d(144, track_running_stats=False)
            self.conv2 = conv3x3(144, 96, bias=bias)
            self.bn2 = nn.BatchNorm2d(96, track_running_stats=False)
        ...
        self.bn3 = nn.BatchNorm2d(96, track_running_stats=False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, downsample=None,expansion=4):
        ...
        self.bn3 = nn.BatchNorm2d(expansion*planes, track_running_stats=False)
...
class MIMO_PreEncoder(nn.Module):
    def __init__(self, in_layer,out_layer,kernel_size=(1,12),dilation=(1,16),use_bn = False):
        ...
        self.bn = nn.BatchNorm2d(out_layer, track_running_stats=False)
    ...
    def _make_layer(self, block, planes, num_blocks):
        if self.use_bn:
            downsample = nn.Sequential(
                ...
                nn.BatchNorm2d(planes * self.block_expansion, track_running_stats=False)
            )
        else:
